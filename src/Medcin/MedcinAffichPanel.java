...
public class MedcinAffichPanel extends JPanel implements ActionListener {
	private static final long serialVersionUID = 1L;
	public static int cpt1 = 0 , cpt2 = 0;
	...
	private JLabel Date;
	private JLabel l2 ;
	private SaveClick myS ;
	
	
	public MedcinAffichPanel(Patient patient)
	{
	...
	private void Init(Patient patient)
	{
		...
		c1 = new JComboBox<>(s2);
		c1.setSelectedIndex((((int)(patient.getSexe())-65)/7)^1);
		
		String s3[] = {"O-","O+","A-","A+","B-","B+","AB-","AB+"};
		c2 = new JComboBox<>(s3);
		c2.setSelectedItem(patient.getGroupSanguin());
		
		
		String str[] = Maladie.maladieToString(Maladie.MaladieAvecTypeUnique(MainFrame.medcin.avoirToutesLesMaladies()
				, TypeMaladie.Simple));
		...
		str =  Maladie.maladieToString(Maladie.MaladieAvecTypeUnique(MainFrame.medcin.avoirToutesLesMaladies()
				, TypeMaladie.Chronique));
		maladieChroniqueCheckList = new CheckList(str);
		
		System.out.println(patient.getMaladies());
		...
		tprenom.setText(patient.getPrenom());
		ttel.setText(patient.getTel());
		tcodePatient.setText(""+patient.getCodePatient());
		tDateNaiss.setText(patient.getDateNaiss().toString());
		tDateInsc.setText(patient.getDateInsc().toString());
		tadrs.setText(patient.getAdresse());
		tcodePatient.disable();
		temail.setText(patient.getEmail());
		save.addActionListener(this);
		
	}
	
	private void Listeners(Patient patient)
	{
			...
			public void actionPerformed(ActionEvent e)
			{
					...
					||  tadrs.getText().isEmpty()))
				{
					Patient p = new Patient(tnom.getText(),tprenom.getText(),ttel.getText(),
							temail.getText(),Integer.parseInt(tcodePatient.getText()),
							c2.getSelectedItem().toString(),tadrs.getText(),
							Classes.Date.StringtoDate(tDateNaiss.getText()),
							Classes.Date.StringtoDate(tDateInsc.getText()),
							c1.getSelectedItem().toString().charAt(0),null,null);			        
					/*
					MainFrame.medcin.effacerPatient(p);
					MainFrame.medcin.ajouterUnNouveauPatient(p);
					MainFrame.medcin.updatePersonne(patient.getNom(), patient.getPrenom(),p.getEmail(),p.getTel(), p.getNom(), p.getPrenom());
				*/
					Medcin.modifierPersonne(patient, p);
			        ...
			        for (int i = 0; i < n; i++) 
			        {
			        	...
			        	if (item.isSelected()) 
			        	{
			        		MainFrame.medcin.assossierMaladieAuPatient(new Maladie(item.toString(),
			        ...
			        AgendaTest.t.repaintTableV5(Fun.PersonneListToTableModel(MainFrame.medcin.getSecretaire().getRdvs()));
		...
		maladieButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cpt1 == 0)
				{
					...
					ListModel<?> model = maladieCheckList.list.getModel();
			        int n = model.getSize();
			        ArrayList<Boolean> CopieMaladie = new ArrayList<Boolean>();
			        
			        // Init Copie //
			        
			        for ( int i = 0 ; i < n ; i++ )
			        {
			        	CopieMaladie.add(false);
			        }
			        CheckableItem item ; 
			        for ( int j = 0 ; j < patient.getMaladies().size() ; j++) 
			        {
			        	for (int i = 0; i < n; i++) 
			            {
			            	item = (CheckableItem) model.getElementAt(i);
			            	if (patient.getMaladies().get(j).egale(item.toString())) 
			            	{
			            		if (patient.getMaladies().get(j).getTypeMaladie() == TypeMaladie.Simple)
			            		{
			            			CopieMaladie.set(i, true);
			            		}
			            	}
			            }
			        }
			        ...
			        CheckListRenderer.setChecked(CopieMaladie);
			        String str2[] = Maladie.maladieToString(Maladie.MaladieAvecTypeUnique(MainFrame.medcin.avoirToutesLesMaladies()
							, TypeMaladie.Simple));
			        maladieCheckList =  new CheckList(str2);
			        cpt1++;
				}
		...
		maladieChroniqueButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cpt2 == 0)
				{
			        ...
			        int n = model.getSize();
			        ArrayList<Boolean> CopieMaladieChronique = new ArrayList<Boolean>();
			        
			        // Init Copie //
			        
			        for ( int i = 0 ; i < n ; i++ )
			        {
			        	CopieMaladieChronique.add(false);
			        }
			        CheckableItem item ; 
			        for ( int j = 0 ; j < patient.getMaladies().size() ; j++) 
			        {
			        	for (int i = 0; i < n; i++) 
			            {
			            	...
			            	if (patient.getMaladies().get(j).egale(item.toString())) 
			            	{
			            		if (patient.getMaladies().get(j).getTypeMaladie() == TypeMaladie.Chronique)
			            		{
			            			CopieMaladieChronique.set(i, true);
			            		}
			            	}
			            }
			        }
			        ...
			        CheckListRenderer.setChecked(CopieMaladieChronique);
			        String str2[] = Maladie.maladieToString(Maladie.MaladieAvecTypeUnique(MainFrame.medcin.avoirToutesLesMaladies()
							, TypeMaladie.Chronique));
			        maladieChroniqueCheckList =  new CheckList(str2);
			        cpt2++;
				}
	...
	public void repaintPanel(Patient patient)
	{
		removeAll();
		updateUI();
		InitTool();
